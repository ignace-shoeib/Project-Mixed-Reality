...
namespace Inworld.Editor
{
    ...
    public partial class InworldEditor : EditorWindow
    {
        ...
        Dictionary<InworldEditorStatus, EditorState> m_States = new Dictionary<InworldEditorStatus, EditorState>();
        Dictionary<InworldWorkspaceData, WorkspaceFetchingProgress> m_Progresses = new Dictionary<InworldWorkspaceData, WorkspaceFetchingProgress>();
        #endregion
        ...
        void _DrawBanner() => rootVisualElement.Add(InworldAI.UI.Header.Instantiate());
        ...
        {
            string fileLocation = _GetFileLocation(InworldAI.Settings.CharacterDataPath);
            Object[] results = AssetDatabase.LoadAllAssetsAtPath(fileLocation);
            foreach (Object obj in results)
            {
                if (obj is not InworldCharacterData characterData)
                    continue;
        ...
        InworldSceneData _GetIwSceneData(string mrID, out int index)
        {
            ...
            foreach (Object obj in results)
            {
                if (obj is not InworldSceneData sceneData)
                    continue;
        ...
        InworldWorkspaceData _GetWSData(string mrID, out int index)
        {
            ...
            foreach (Object obj in results)
            {
                if (obj is not InworldWorkspaceData wsData)
                    continue;
