    ...
    public class EditorSceneChooser : EditorState
    {
        ...
        protected override void _SetupContents()
        {
            ...
            if (string.IsNullOrEmpty(targetKey))
                ...
                m_KeyChooser = SetupDropDown("KeyChooser", InworldAI.Game.currentWorkspace.integrations.Select(key => key.ShortName).ToList(),
                                             OnKeyChanged, targetKey);
            if (string.IsNullOrEmpty(targetScene))
                m_SceneChooser = SetupDropDown("SceneChooser", null, OnSceneChanged, null, false);
            else
                m_SceneChooser = SetupDropDown("SceneChooser", InworldAI.Game.currentWorkspace.scenes.Select(scene => scene.ShortName).ToList(),
