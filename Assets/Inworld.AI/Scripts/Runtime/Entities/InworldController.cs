...
namespace Inworld
{
    public class InworldController : SingletonBehavior<InworldController>
    {
        ...
        /// <summary>
        /// Get/Set the current Inworld Character.
        /// Usually it's set by CheckPriority().
        /// </summary>
        public InworldCharacter CurrentCharacter
        {
            get => m_CurrentCharacter ? m_CurrentCharacter : Characters[0];
            private set
            {
        ...
        /// </summary>
        static ControllerStates State
        {
            get => Instance.m_State;
            set => Instance._SetState(value); 
        }
...
#pragma warning restore CS4014
        ...
        void _InitCharacters()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                {
                    InworldCharacter tmp = transform.GetChild(i).GetComponent<InworldCharacter>();
                    if (!tmp)
                        continue;
                    Characters.Add(tmp);
                }
        ...
        void _BindCharacterFromServer(InworldCharacter character,
            ...
            while (State == ControllerStates.Connected)
            {
                ...
                if (!m_Client.IsInteracting)
                {
                    if (!m_Client.Errors.IsEmpty)
                    {
                        ...
                        m_BackOffTime = UnityEngine.Random.Range(m_BackOffTime, m_BackOffTime * 2);
...
#pragma warning restore 4014
        ...
        {
            Packets.ControlEvent controlEvent = new Packets.ControlEvent(Grpc.ControlEvent.Types.Action.TtsPlaybackStart, Routing.FromAgentToPlayer(ID));
        ...
        {
            Packets.ControlEvent controlEvent = new Packets.ControlEvent(Grpc.ControlEvent.Types.Action.TtsPlaybackEnd, Routing.FromAgentToPlayer(ID));
            controlEvent.PacketId.InteractionId = m_TTSInteractionID;
            SendEvent(controlEvent);
        }
        #endregion
        ...
        public async Task LoadCharacter(InworldCharacter character)
        {
            try
            {
                Debug.Log("InworldController Connecting.");
        ...
        public async Task LoadScene(InworldSceneData inworldSceneData)
        {
            try
            {
                InworldAI.Log("InworldController Connecting.");
        ...
        /// </summary>
        /// <param name="characterID">string of Character ID, would be generated only after InworldScene is loaded and session is started</param>
