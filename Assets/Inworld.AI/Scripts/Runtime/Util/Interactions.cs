...
namespace Inworld
{
    ...
    class Interactions
    {
        ...
        readonly InteractionEvent m_Event;
        ...
        Packets.PacketId m_CurrentUtteranceID;
        public InteractionEvent Event => m_Event;
        ...
        internal void AddInteractionEnd(string interactionId)
        {
            ...
            if (m_PlayedUtterances.ContainsKey(lastHistoryItem.UtteranceId)
                && (m_CurrentUtteranceID == null || m_CurrentUtteranceID.UtteranceId != lastHistoryItem.UtteranceId))
            {
                // Already played utterance.
                CompleteInteraction(interactionId);
            }
        }
        internal Packets.CancelResponsesEvent AddText(Packets.TextEvent text)
        {
            if (IsInteractionCanceled(text.PacketId.InteractionId))
            {
                Debug.Log($"Cancel followup: {text.PacketId.InteractionId}");
                return new Packets.CancelResponsesEvent(text.PacketId.InteractionId,
                    new List<string>() {text.PacketId.UtteranceId});
            }
            Packets.CancelResponsesEvent cancel = null;
            if (m_CurrentUtteranceID != null && text.Routing.Source.IsPlayer() &&
                text.PacketId.InteractionId != m_CurrentUtteranceID.InteractionId)
            {
                ...
                if (canceledUtterances.Count > 0)
                    cancel = new Packets.CancelResponsesEvent(m_CurrentUtteranceID.InteractionId, canceledUtterances);
        ...
        internal void StartUtterance(Packets.PacketId packetId)
        {
            m_CurrentUtteranceID = packetId;
            m_PlayedUtterances.Add(packetId.UtteranceId, true);
            OnChatHistoryListChanged();
        }
        internal void CompleteUtterance(Packets.PacketId packetId)
        {
        ...
        {
            m_Event.Invoke(InteractionStatus.HistoryChanged, History);
        }
        HistoryItem FindChatHistoryItemByUtteranceId(string utteranceId) =>
            m_ChatHistoryByUtteranceID.ContainsKey(utteranceId) ? m_ChatHistoryByUtteranceID[utteranceId] : null;
        internal void CompleteInteraction(string interactionId)
        {
            ...
            m_Event.Invoke(InteractionStatus.InteractionCompleted, itemsByInteraction);
        }
        #endregion
    }
}